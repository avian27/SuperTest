...
function generateCodeNameString(stringText){
    const randomPosition = faker.random.numeric({ min: 0, max: 2 });
    let prefix = '';
    let suffix = '';
    if (randomPosition === 0) {
        suffix = faker.random.alphaNumeric(stringText.length);
    } else if (randomPosition === 1) {
        prefix = faker.random.alphaNumeric(faker.random.numeric({min: 0, max: stringText.length}));
        suffix = faker.random.alphaNumeric(faker.random.numeric({min: 0, max: stringText.length}));
    } else {
        prefix = faker.random.alphaNumeric(stringText.length);
    }
...
function tagsArrayData(){
    const tagsArraySize = faker.datatype.number({min:0,max:15});
    for(let i=0; i<tagsArraySize; i++){
        tagsArray.push(faker.random.alpha(5));
    }
    return tagsArray;
}
function allowedValuesData(){
    const allowedValuesSize = faker.datatype.number({min:0,max:15});
    for(let i=0; i<allowedValuesSize; i++){
        allowedValuesArray.push(faker.random.alpha(7));
    }
    return allowedValuesArray;
}

function generateDescString(){
    const stringWordCap = faker.datatype.number({min:0,max:100});
    for(let i=0; i<stringWordCap; i++){
        descriptionString = `${descriptionString} ${faker.random.alpha(faker.datatype.number({min:0,max:10}))}`;
    }
...
export function createShortTextAttributePayload(stringText){
    ...
    const payload ={
        ...
        limit: faker.datatype.number({min:1,max:200}),
...
export function createDecimalAttributePayload(stringText){
    const tagsData = tagsArrayData();
    const descriptionString = generateDescString();
    const maxVal = faker.datatype.number({min:-100000,max:100000, precision: 0.01});
    const minVal = faker.datatype.number({min:-100000,max:maxVal, precision: 0.01});
    const generatedName = generateCodeNameString(stringText);
    const generatedCode = generateCodeNameString(stringText);
    const payload ={
        active: booleanStates[Math.floor(Math.random() * booleanStates.length)],
...
export function createFileAttributePayload(stringText){
    ...
    const fileAllowedArraySize = faker.datatype.number({min:0,max:6});
    ...
    const payload ={
        ...
        max_size: faker.datatype.number({min:1,max:200}),
        allow_multiple: booleanStates[Math.floor(Math.random() * booleanStates.length)],
        vms_visible: booleanStates[Math.floor(Math.random() * booleanStates.length)],
        vms_editable: booleanStates[Math.floor(Math.random() * booleanStates.length)],
        store_filter: booleanStates[Math.floor(Math.random() * booleanStates.length)],
...
export function createMediaAttributePayload(stringText){
    ...
    const payload ={
        ...
        allowed_extensions: fileAllowedArray,
        max_size: faker.datatype.number({min:1,max:10000}),
        allow_multiple: booleanStates[Math.floor(Math.random() * booleanStates.length)],
        vms_visible: booleanStates[Math.floor(Math.random() * booleanStates.length)],
        vms_editable: booleanStates[Math.floor(Math.random() * booleanStates.length)],
        store_filter: booleanStates[Math.floor(Math.random() * booleanStates.length)],
