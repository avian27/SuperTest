...
export function createOrgPayload(org) {
    ...
    const payloadObj = {
        ...
        categoriesCount: faker.number.int({ min: 1, max: 20 }),
...
export async function fetchOrgId(baseURL, cookie) {
    ...
    const res = await request
        ...
        .set(headers);
    const randomOrgIndex = faker.number.int({ min: 0, max: res.body.length });
...
export async function fetchBrandOrgId(baseURL, cookie) {
    ...
    const randomOrgIndex = faker.number.int({ min: 0, max: temp.length });
...
export function inviteMembersPayload(emailArraySize, userGroupSize) {
    ...
    /*for(let i=0; i<userGroupSize; i++){
        userGroupArray.push(faker.string.alpha(15));
    }*/
    const payloadObj = {
