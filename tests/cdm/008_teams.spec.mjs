...
describe('Teams and User Test cases @cdm @inviteMember', async () => {
    before(async () => {
        orgId = await fetchOrgId(baseUrl, cookie);
    });

    describe('Invite Member @cdm @inviteMember', async () => {

        it('Get Invited Members by OrgId - User is logged in', async () => {
        ...
        it('Invite Member - User is not logged in', async () => {
        ...
        it('Invite Member - User is logged in but invalid orgId is sent', async () => {
        ...
        it('Invite Member - User is logged in', async () => {
            endpoint = inviteMemberEndpoints.inviteMember(orgId);
            payloadData = inviteMembersPayload(faker.datatype.number({ min: 1, max: 15 }), faker.datatype.number({ min: 1, max: 10 }))
            payload = JSON.stringify({
                "emails": payloadData.email,
        ...
        it('Get Invite Member by OrgId - User is not logged in', async () => {
            endpoint = inviteMemberEndpoints.getInviteMemberByOrgId(orgId);
            payload = null;
            signed_headers = getSignedRequestHeaders("GET", baseUrl, endpoint, payload, {});
            headers = {
        ...
        it('Get Invite Member by OrgId - Invalid orgId is sent', async () => {
