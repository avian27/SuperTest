...
describe('VendorMapping Api Test Cases @cdm @VendorMap', async () => {
    before(async () => {
        orgId = FO.getValueFromFile(attributeDetailsFile, 'OrgId');
    });

    describe('Get Vendor List @cdm @VendorMap', async () => {
        it('Get All Vendors - user is not logged in', async () => {
            payload = null;
        ...
        it('Get All Vendors - user is logged in', async () => {
            payload = null;
            endpoint = getVendorListEndpoint(orgId);
            signed_headers = getSignedRequestHeaders("GET", baseUrl, endpoint, payload, {});
            headers = {
        ...
        it('Get All Vendors - user is logged in but invalid Vendor Id is sent', async () => {
            payload = null;
        ...
        it('Get All Vendors user is logged in but vendor Id is sent in request', async () => {
            payload = null;
    ...
    describe('Adding Vendors to Brand @cdm @VendorMap', async () => {
        it('Add vendors to brand - user is not logged in', async () => {
            endpoint = createVendorMappingEndpoint(orgId);
            const tempVendorArrSize = faker.datatype.number({ min: 0, max: vendorList.length });
            const tempVendorArr = [];
        ...
        it('Add vendors to brand - user is logged in', async () => {
            endpoint = createVendorMappingEndpoint(orgId);
            const tempVendorArrSize = faker.datatype.number({ min: 0, max: vendorList.length });
            const tempVendorArr = [];
            for (let i = 0; i < tempVendorArrSize; i++) {
        ...
        it('Add vendors to brand - Vendors Id which are mapped already are sent', async () => {
            endpoint = createVendorMappingEndpoint(orgId);
            payload = JSON.stringify({
        ...
        it('Add vendors to brand - empty array is sent', async () => {
            endpoint = createVendorMappingEndpoint(orgId);
            payload = JSON.stringify({
                vendorIds: []
            });
    ...
    describe('Get Vendor Mapping List @cdm @VendorMap', async () => {
        it('Get Vendors mapped to a particular Brand - User is not logged in', async () => {
            payload = null;
        ...
        it('Get Vendors mapped to a particular Brand - User is logged in', async () => {
            payload = null;
            endpoint = getVendorMapListEndpoint(orgId);
            signed_headers = getSignedRequestHeaders("GET", baseUrl, endpoint, payload, {});
            headers = {
        ...
        it('Get Vendors mapped to a particular Brand - User is logged in invalid orgId is provided', async () => {
            payload = null;
            invalidOrgId = orgId.substring(0, orgId.length - 1);
        ...
        it('Get Vendors mapped to a particular Brand - User is logged in but vendor Id is provided', async () => {
            payload = null;
            invalidOrgId = await fetchVendorOrgId(baseUrl, cookie);
            endpoint = getVendorMapListEndpoint(invalidOrgId);
            signed_headers = getSignedRequestHeaders("GET", baseUrl, endpoint, payload, {});
